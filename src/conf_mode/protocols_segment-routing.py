...
def get_config(config=None):
    ...
    if dict_search('traffic_engineering.segment_list', sr):
        for segment_list in sr['traffic_engineering']['segment_list']:
            for index in sr['traffic_engineering']['segment_list'][segment_list]['index']['value']:
                if 'adjacency' in (dict_search(f'traffic_engineering.segment_list.' \
                                               f'{segment_list}.index.value.{index}.nai', sr)) \
                and 'prefix' in (dict_search(f'traffic_engineering.segment_list.' \
                                             f'{segment_list}.index.value.{index}.nai', sr)):
    ...
    if dict_search('traffic_engineering.segment_list', sr):
        for segment_list in sr['traffic_engineering']['segment_list']:
            for index in sr['traffic_engineering']['segment_list'][segment_list]['index']['value']:
                if 'ipv4' in (dict_search(f'traffic_engineering.segment_list.' \
                                               f'{segment_list}.index.value.{index}.nai.adjacency', sr)) \
                and 'ipv6' in (dict_search(f'traffic_engineering.segment_list.' \
                                             f'{segment_list}.index.value.{index}.nai.adjacency', sr)):
                    raise ConfigError(f'Segment routing traffic engineering segment list ' \
    ...
    if dict_search('traffic_engineering.segment_list', sr):
        for segment_list in sr['traffic_engineering']['segment_list']:
            for index in sr['traffic_engineering']['segment_list'][segment_list]['index']['value']:
                if 'ipv4' in (dict_search(f'traffic_engineering.segment_list.' \
                                               f'{segment_list}.index.value.{index}.nai.prefix', sr)) \
                and 'ipv6' in (dict_search(f'traffic_engineering.segment_list.' \
                                             f'{segment_list}.index.value.{index}.nai.prefix', sr)):
                    raise ConfigError(f'Segment routing traffic engineering segment list ' \
